    ...
    fn encode(self: *Keyboard, keycode: u32, buf: []u8) ?[]const u8 {
        ...
        switch (sym) {
            c.XKB_KEY_Return, c.XKB_KEY_KP_Enter => {
                // Shift+Enter and Ctrl+Enter are LF. Legacy has no way to encode
                // either modifier on this key, so the convention is to send the
                // *other* newline byte — which is what a full-screen editor reads as
                // "break the line rather than accept it", Claude Code's prompt among
                // them. Shift is the one that matters in practice: it is the chord
                // Claude Code documents, and its `/terminal-setup` installs exactly
                // this binding in the terminals that let it. Nothing is lost in a
                // shell, where the line discipline accepts LF and CR alike, so both
                // chords still submit at a prompt, exactly like Ctrl+J.
                if (shift or ctrl) return copy(buf, if (alt) "\x1b\n" else "\n");
